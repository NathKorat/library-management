...
/**
 * The Library.Management.Borrower interface must be implemented by the Library.Management.Student and Library.Management.Faculty class
 * 
 */
public class Borrower extends LibraryUser{
    ...
    void viewRequests(){
			...
			LibraryResource res;
			Library lib = Library.getInstance("LUMS Library.Management.Library");
     ...
     * Remember: for checking availability, this function should call the checkAvailability() method in the Library.Management.Borrowable interface
     * for actually issueing the resource, this function will call the issueResource() method in the Library.Management.Borrowable interface
     * @param res give in the resource id of the Library.Management.Book/ReadingPack which needs to be issued
    ...
    boolean tryIssue(ArrayList<LibraryResource> res){
			...
			for(int i=0;i<res.size();i++){
				if(res.get(i).type == Constants.MAGAZINE){
						System.out.println("A Library.Management.Magazine cannot be issued!");
					return false;
				}
				...
				if(maxResources <= issuedResources.size() || !borrowable.checkStatus()){
					...
					if(maxResources <= issuedResources.size()){
						...
						System.out.println("The requested resource to another Library.Management.Library user.");
					check = true;
				}
				else{
					check = false;
    ...
    /**
     *
     * @param resourceID give in the resource id of the Library.Management.Book/ReadingPack which needs to be returned
     ...
     * for returning the resource, you must call the returnResource() method of the Library.Management.Borrowable interface
    ...
    boolean tryReturn(int resourceID){
			...
			for(Fine fine: fines){
				if(fine.fineID == relatedFineID){
					...
					if(fine.fine == 0){										// If there was no fine, then remove this object<Library.Management.Fine>
    ...
    /**
     *
     * @param resourceID the unique resourceID of the Library.Management.Book/Library.Management.CoursePack whose pending request we want to withdraw
